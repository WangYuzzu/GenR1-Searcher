...
def train(args):
    ...
    if args.colocate_actor_ref:                      # --colocate_actor_ref
        assert (
            args.actor_num_nodes == args.ref_num_nodes and
            args.actor_num_gpus_per_node == args.ref_num_gpus_per_node
        ), "若要同卡共存，Actor 与 Ref 的节点/卡数必须一致"

        bundles = [                                  # 每个节点预留 GPU+CPU 资源
    ...
    if args.critic_pretrain and args.colocate_critic_reward:
        assert (
            args.critic_num_nodes == args.reward_num_nodes and
            args.critic_num_gpus_per_node == args.reward_num_gpus_per_node
        ), "Critic 与 Reward 共卡时节点/卡数必须一致"
    ...
    # ------------ Critic（可选：只有 --critic_pretrain 时启动） ------------
    if args.critic_pretrain:
        critic_model = PPORayActorGroup(
            args.critic_num_nodes,
            args.critic_num_gpus_per_node,
    ...
    # ------------ Reward Model（可选：若不用远端 FastAPI） ------------
    if not args.remote_rm_url:
        reward_pretrains = args.reward_pretrain.split(",")   # 支持多支 RM
        reward_models = []
        for _ in reward_pretrains:
            reward_models.append(
                PPORayActorGroup(
    ...
    if args.vllm_num_engines and args.vllm_num_engines > 0:
        max_len = args.max_len or (args.prompt_max_len + args.generate_max_len)
        vllm_engines = create_vllm_engines(
            args.vllm_num_engines,
            args.vllm_tensor_parallel_size,
            args.pretrain,
    ...
    # --------------------- 初始化 Critic（若有） ---------------------
    if args.critic_pretrain:
        # 先向任一 Actor 查询 max_steps，再用它来初始化 Critic 的 scheduler
        max_steps = ray.get(actor_model._actor_handlers[0].max_steps.remote())
        refs.extend(
            critic_model.async_init_model_from_pretrained(strategy,
    ...
    refs = actor_model.async_fit_actor_model(
        ...
        ref_model,
        reward_models,
        args.remote_rm_url,          # 若有 FastAPI，用它取 reward
        reward_fn=reward_fn,
        vllm_engines=vllm_engines,
    )
    ray.get(refs)        # 等所有 Actor 完成训练
