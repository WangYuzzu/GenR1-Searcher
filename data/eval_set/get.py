...
def analyze_results(file_path):
    ...
    for item in results:
        ...
        if calculate_exact_match(pred_ans, gold_ans):
            ...
            if item.get('search_count') >= 1 and item.get('gendoc_count') >= 1:
                print(f'search count: {item.get("search_count")}, gendoc count: {item.get("gendoc_count")}')
                print(item.get('idx'), item.get('question'))
...
if __name__ == "__main__":
    file_path = "hotpotqa_500-ckptstep350_two_tools_temp0.0_typeNone.jsonl"
